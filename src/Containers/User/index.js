...
const mapStateToProps = (state, ownProps) => {
  ...
  const allDataArr = UserSelectors.getAllDataArr(state.user)
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(UserSelectors.getMaxModifiedon(state.user) || 0),
    allDataArr,
    allIds: UserSelectors.getAllIds(state.user),
    byId: UserSelectors.getById(state.user),
    entityName: 'User',
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    fetchAll: query => dispatch(UserActions.userRequestAll(query)),
