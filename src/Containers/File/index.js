        ...
        accessor: o => {
          return (
            <div>
              <ButtonAction
                as={'link'}
                to={`/entity/filecontent?file=${o._id}`}
                viewButton
              />
              <ButtonAction
                as={'link'}
...
const mapStateToProps = (state, ownProps) => {
  ...
  const allDataArr = FileSelectors.getAllDataArr(state.file)
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(FileSelectors.getMaxModifiedon(state.file) || 0),
    allDataArr,
    allIds: FileSelectors.getAllIds(state.file),
    byId: FileSelectors.getById(state.file),
    entityName: 'File',
...
const mapDispatchToProps = dispatch => {
  return {
    ...
    fetchAll: query => dispatch(FileActions.fileRequestAll(query)),
    deleteRow: query => dispatch(FileActions.fileDeleteSuccess(query)),
    updateOne: (data, id) => dispatch(FileActions.fileUpdate(data, id)),
    removeOne: (data, id) => dispatch(FileActions.fileRemove(data, id)),
    updateBatch: data => dispatch(FileActions.fileUpdateBatch(data))
  }
}
