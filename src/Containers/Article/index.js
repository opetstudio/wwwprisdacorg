...
const mapStateToProps = (state, ownProps) => {
  ...
  const allDataArr = ArticleSelectors.getAllDataArr(state.article)
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(ArticleSelectors.getMaxModifiedon(state.article) || 0),
    allDataArr,
    allIds: ArticleSelectors.getAllIds(state.article),
    byId: ArticleSelectors.getById(state.article),
    entityName: 'Article',
    isLoggedIn: LoginSelectors.isLoggedIn(state.login),
    loginToken: LoginSelectors.getToken(state.login),
    filter,
    pageCount: ArticleSelectors.getPageCount(state.article),
    pageSize: ArticleSelectors.getPageSize(state.article)
  }
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchAll: (query) => dispatch(ArticleActions.articleRequestAll(query)),
    deleteRow: (query) => dispatch(ArticleActions.articleDeleteSuccess(query)),
    updateOne: (data, id) => dispatch(ArticleActions.articleUpdate(data, id)),
    removeOne: (data, id) => dispatch(ArticleActions.articleRemove(data, id)),
    updateBatch: (data) => dispatch(ArticleActions.articleUpdateBatch(data))
  }
}
