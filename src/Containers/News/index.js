...
const mapStateToProps = (state, ownProps) => {
  ...
  const allDataArr = NewsSelectors.getAllDataArr(state.news)
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(NewsSelectors.getMaxModifiedon(state.news) || 0),
    allDataArr,
    allIds: NewsSelectors.getAllIds(state.news),
    byId: NewsSelectors.getById(state.news),
    entityName: 'News',
    isLoggedIn: LoginSelectors.isLoggedIn(state.login),
    loginToken: LoginSelectors.getToken(state.login),
    filter,
    pageCount: NewsSelectors.getPageCount(state.news),
    pageSize: NewsSelectors.getPageSize(state.news)
  }
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchAll: (query) => dispatch(NewsActions.newsRequestAll(query)),
    deleteRow: (query) => dispatch(NewsActions.newsDeleteSuccess(query)),
    updateOne: (data, id) => dispatch(NewsActions.newsUpdate(data, id)),
    removeOne: (data, id) => dispatch(NewsActions.newsRemove(data, id)),
    updateBatch: (data) => dispatch(NewsActions.newsUpdateBatch(data))
  }
}
