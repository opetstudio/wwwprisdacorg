...
const mapStateToProps = (state, ownProps) => {
  ...
  const allDataArr = ProductSelectors.getAllDataArr(state.product)
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(ProductSelectors.getMaxModifiedon(state.product) || 0),
    allDataArr,
    allIds: ProductSelectors.getAllIds(state.product),
    byId: ProductSelectors.getById(state.product),
    entityName: 'Product',
    isLoggedIn: LoginSelectors.isLoggedIn(state.login),
    loginToken: LoginSelectors.getToken(state.login),
    filter,
    pageCount: ProductSelectors.getPageCount(state.product),
    pageSize: ProductSelectors.getPageSize(state.product)
  }
}
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchAll: (query) => dispatch(ProductActions.productRequestAll(query)),
    deleteRow: (query) => dispatch(ProductActions.productDeleteSuccess(query)),
    updateOne: (data, id) => dispatch(ProductActions.productUpdate(data, id)),
    removeOne: (data, id) => dispatch(ProductActions.productRemove(data, id)),
    updateBatch: (data) => dispatch(ProductActions.productUpdateBatch(data))
  }
}
