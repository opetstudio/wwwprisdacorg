...
export function * getProduct (api, action) {
  console.log('[ProductSaga] getProduct action=', action)
  const { data } = action
  const s = yield select(session)
  // make the call to the api
  const response = yield call(api.getProduct, data, {session: s})
  ...
  if (response.ok) {
    ...
    yield put(ProductActions.productRequestSuccess({requestMessage: 'success fetch data', byId, allIds}))
  } else {
    yield put(ProductActions.productRequestFailed({requestMessage: 'failed fetch data'}))
  }
}
export function * postProduct (api, action) {
  const { data } = action
  // make the call to the api
  const s = yield select(session)
  const response = yield call(api.postProduct, data, {session: s})
  ...
  if (response.ok) {
    ...
    if (response.status === 201) {
      ...
      yield put(ProductActions.productCreateSuccess({formSubmitMessage: 'success post data', byId, allIds}))
    } else yield put(ProductActions.productCreateFailed({ formSubmitMessage: 'Failed create data' }))
  } else {
    ...
    yield put(ProductActions.productCreateFailed({ formSubmitMessage: validationMessages }))
  }
}
...
export function * updateProduct (api, action) {
  ...
  const s = yield select(session)
  const response = yield call(api.updateProduct, data, id, {session: s})
  if (path(['originalError', 'response', 'status'], response) === 401 && path(['originalError', 'response', 'statusText'], response) === 'Unauthorized') return yield put(LoginActions.loginRemoveSuccess({}))
  // success?
  if (response.ok) {
    ...
    //   yield put(ProductActions.productDeleteSuccess({listId: allDeletedIds}))
    // } else yield put(ProductActions.productUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
    yield put(ProductActions.productUpdateSuccess({formSubmitMessage: 'success update data', byId, allIds}))
  } else {
    let validationMessages = JSON.stringify(path(['originalError', 'response', 'data', 'validation_messages'], response))
    if (response.status === 500) validationMessages = path(['originalError', 'response', 'data', 'detail'], response)
    yield yield put(ProductActions.productUpdateFailed({ formSubmitMessage: validationMessages }))
  }
}
export function * updateProductBatch (api, action) {
  const { data } = action
  // make the call to the api
  // const params = yield select(theData)
  const s = yield select(session)
  const response = yield call(api.updateProductBatch, data, {session: s})
  if (path(['originalError', 'response', 'status'], response) === 401 && path(['originalError', 'response', 'statusText'], response) === 'Unauthorized') return yield put(LoginActions.loginRemoveSuccess({}))
  // success?
  if (response.ok) {
    // const { contentDetail, status, msg } = response.data
...
export function * removeProduct (api, action) {
  const { data, id } = action
  // make the call to the api
  const s = yield select(session)
  const response = yield call(api.removeProduct, data, id, {session: s})
  console.log('removeProduct response=>', response)

  // success?
  if (response.ok) {
    yield put(ProductActions.productRemoveSuccess({id}))
  } else {
    yield put(ProductActions.productRequestFailed({requestMessage: 'failed remove data'}))
  }
}
...
export function * getProducts (api, action) {
  ...
  // make the call to the api
  const s = yield select(session)
  const response = yield call(api.getProducts, data, {session: s})
  console.log('responseeeeee', response)
  if (path(['originalError', 'response', 'status'], response) === 401 && path(['originalError', 'response', 'statusText'], response) === 'Unauthorized') return yield put(LoginActions.loginRemoveSuccess({}))
  ...
  if (response.ok) {
    ...
    const {byId, allIds, maxModifiedon} = getEntityCollection(response.data, 'tb_product')
    let { page_count: pageCount, page_size: pageSize } = response.data
    yield put(ProductActions.productRequestSuccess({requestMessage: 'success fetch data', byId, allIds, maxModifiedon, pageCount, pageSize}))
