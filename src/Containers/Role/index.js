...
const mapStateToProps = (state, ownProps) => {
  ...
  const allDataArr = RoleSelectors.getAllDataArr(state.role)
  ...
  return {
    ...
    defaultPageSize,
    column,
    maxModifiedon: parseInt(RoleSelectors.getMaxModifiedon(state.role) || 0),
    allDataArr,
    allIds: RoleSelectors.getAllIds(state.role),
    byId: RoleSelectors.getById(state.role),
    entityName: 'Role',
...
const mapDispatchToProps = (dispatch) => {
  return {
    ...
    fetchAll: (query) => dispatch(RoleActions.roleRequestAll(query)),
    deleteRow: (query) => dispatch(RoleActions.roleDeleteSuccess(query)),
    updateOne: (data, id) => dispatch(RoleActions.roleUpdate(data, id)),
    removeOne: (data, id) => dispatch(RoleActions.roleRemove(data, id)),
    updateBatch: (data) => dispatch(RoleActions.roleUpdateBatch(data))
  }
}
