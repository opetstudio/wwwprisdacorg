...
class TheComponent extends Component {
  static propTypes = {
    ...
    badgeFetchAll: PropTypes.func.isRequired,
    evaluateParticipant: PropTypes.func.isRequired,
    badgeAllDataArr: PropTypes.array.isRequired,
  ...
  constructor (props) {
    ...
    this.state = {
      ...
      userById: userById,
      allDataInspectArr: this.props.allDataInspectArr,
      searchString: '',
      badgeAllDataArr: badgeAllDataArr,
      columnModified: this.props.column
    }
  ...
  _setupColumn (column, { updateOne, badgeAllDataArr = [] }) {
    ...
    if (
      ...
      // console.log('=====>badge====>', this.state.badgeAllDataArr)
      badgeAllDataArr.map(badge => {
        if (badge.status === 'publish') {
          columnTable[0].columns.push({
  ...
  componentDidUpdate (prevProps, prevState) {
    ...
    if (
      ...
      this.setState({
        badgeAllDataArr
        // column: this._setupColumn(this.props.column, {updateOne: this.props.updateOne, badgeAllDataArr})
        // column: this._setupColumn(this.props.column, {updateOne: this.props.updateOne, badgeAllDataArr})
      })
    }
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    filter,
    badgeAllDataArr: BadgeSelectors.getAllDataArr(state.badge)
  }
}
